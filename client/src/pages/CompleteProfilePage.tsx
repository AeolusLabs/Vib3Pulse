...
import { UserPlusIcon, UsersIcon, Building2Icon, ArrowRightIcon, ArrowLeftIcon, CheckIcon, Loader2Icon } from "@/components/ui/icons";
...
export default function CompleteProfilePage() {
  ...
  const socialForm = useForm<SocialData>({
    ...
    defaultValues: { displayName: sessionUser?.displayName || "", dateOfBirth: "", gender: undefined, bio: "", interests: [] },
  });

  const organizerForm = useForm<OrganizerData>({
    resolver: zodResolver(organizerSchema),
    defaultValues: { organizationName: "", bio: "", contactEmail: sessionUser?.email || "", socialMediaLinks: [], canManageVenues: false },
  });

  useEffect(() => {
    if (sessionUser) {
      socialForm.reset({ displayName: sessionUser.displayName || "", dateOfBirth: "", gender: undefined, bio: "", interests: [] });
      organizerForm.reset({ organizationName: "", bio: "", contactEmail: sessionUser.email || "", socialMediaLinks: [], canManageVenues: false });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sessionUser?.id]);

  const onRoleSubmit = (data: RoleData) => {
    setRoleData(data);
    setStep("profile");
  };

  const onFinalSubmit = async (data: SocialData | OrganizerData) => {
    if (!roleData) return;
    setIsLoading(true);
    try {
  ...
  if (sessionLoading) {
    return (
      <div className="min-h-screen bg-[#090909] flex items-center justify-center">
        <Loader2Icon className="h-8 w-8 animate-spin text-violet-400" />
      </div>
    );
  }
      ...
      <div className="relative z-10 flex-1 flex items-start justify-center px-6 py-10">
        <div className="w-full max-w-lg">
          <div className="flex items-center gap-0 mb-12">
            {["Role", "Profile"].map((label, i) => {
              const stepNum = i + 1;
              const currentStepNum = step === "role" ? 1 : 2;
              const done = currentStepNum > stepNum;
              const active = currentStepNum === stepNum;
              return (
                <div key={label} className="flex items-center">
                  <div className="flex flex-col items-center gap-1.5">
                    ...
                    >
                      {done ? <CheckIcon className="w-3.5 h-3.5" /> : stepNum}
                    </motion.div>
                    <span className={`text-[0.6rem] tracking-[0.12em] uppercase font-sans transition-colors duration-300 ${active ? "text-white/60" : done ? "text-violet-400/60" : "text-white/20"}`}>
                      {label}
                    </span>
                  </div>
                  {i === 0 && (
                    <div className={`w-16 sm:w-24 h-px mx-3 mb-5 transition-colors duration-300 ${currentStepNum > 1 ? "bg-violet-600/40" : "bg-white/[0.07]"}`} />
                  )}
                </div>
              );
            })}
          ...
          <div className="mb-8">
            <h1 className="font-serif text-3xl md:text-4xl font-bold text-white tracking-tight leading-tight mb-2">
              {step === "role" ? "One more step." : "Complete your profile."}
            </h1>
            <p className="text-white/35 text-sm font-sans">
              {step === "role"
                ? "Signing up with Google skips a couple of things we still need — how will you use Vib3Pulse?"
          ...
          {step === "role" && (
            <Form {...roleForm}>
              <form onSubmit={roleForm.handleSubmit(onRoleSubmit)} className="space-y-4">
                ...
                />
                <div className="pt-2">
                  <Button type="submit" className="w-full h-12 bg-violet-600 hover:bg-violet-500 text-white border-0 rounded-xl font-sans font-medium text-sm shadow-lg shadow-violet-600/15" data-testid="button-next-role">
          ...
          {step === "profile" && roleData?.userType === "social" && (
            <Form {...socialForm}>
              <form onSubmit={socialForm.handleSubmit(onFinalSubmit)} className="space-y-5">
                <FormField
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormDescription className="text-white/25 text-xs">Can only be changed once.</FormDescription>
                      <FormMessage className="text-xs text-red-400" />
                    </FormItem>
                  )}
                />
                ...
                <div className="flex gap-3 pt-2">
                  <Button type="button" variant="outline" onClick={() => setStep("role")} className="flex-1 h-12 bg-transparent border-white/[0.09] text-white/50 hover:bg-white/[0.05] hover:text-white rounded-xl font-sans text-sm" data-testid="button-back-profile">
                    <ArrowLeftIcon className="w-4 h-4 mr-2" /> Back
                  </Button>
                  <Button type="submit" className="flex-1 h-12 bg-violet-600 hover:bg-violet-500 text-white border-0 rounded-xl font-sans font-medium text-sm shadow-lg shadow-violet-600/15 overflow-hidden relative" disabled={isLoading} data-testid="button-complete-profile">
                    <AnimatePresence mode="wait" initial={false}>
                      {isLoading ? (
                        ...
                        <motion.span key="idle" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.16, ease: [0.22, 1, 0.36, 1] }} className="flex items-center">
                          <UserPlusIcon className="w-4 h-4 mr-2" />Finish
                        </motion.span>
                      )}
                    </AnimatePresence>
                  </Button>
          ...
          {step === "profile" && roleData?.userType === "organizer" && (
            <Form {...organizerForm}>
