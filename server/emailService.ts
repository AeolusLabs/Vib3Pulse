...
import type { SupportedCurrency } from './payments/types.js';
...
interface SendTicketPurchaseEmailParams {
  to: string;
  userName?: string;
  eventTitle: string;
  eventDate: Date;
  ...
  tierName?: string;
  quantity: number;
  amountPaid: number; // smallest currency unit, total across all tickets in this order
  currency: SupportedCurrency;
  walletLink: string;
}

export async function sendTicketPurchaseEmail({
  to, userName, eventTitle, eventDate, eventLocation, tierName, quantity, amountPaid, currency, walletLink,
}: SendTicketPurchaseEmailParams): Promise<boolean> {
  try {
    const formattedDate = eventDate.toLocaleDateString('en-GB', {
      weekday: 'long', day: 'numeric', month: 'long', year: 'numeric',
    });
    const formattedTime = eventDate.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });
    const ticketLabel = quantity > 1 ? `${quantity} tickets` : '1 ticket';
    const total = formatAmount(amountPaid, currency);

    const { data, error } = await resend.emails.send({
      from: `${APP_NAME} <${FROM_EMAIL}>`,
      to: [to],
      subject: `Your ticket${quantity > 1 ? 's' : ''} for ${eventTitle}`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #ffffff; padding: 30px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 10px 10px;">
            <h2 style="color: #333; margin-top: 0;">You're going! 🎟️</h2>
            <p>Hi${userName ? ` ${userName}` : ''},</p>
            <p>Your purchase is confirmed. Here's your order summary:</p>
            <div style="background: #f7f5ff; border-radius: 8px; padding: 20px; margin: 20px 0;">
              <p style="margin: 0 0 8px; font-size: 18px; font-weight: bold; color: #333;">${eventTitle}</p>
              <p style="margin: 0 0 4px; color: #555;">${formattedDate} at ${formattedTime}</p>
              <p style="margin: 0 0 12px; color: #555;">${eventLocation}</p>
              <hr style="border: none; border-top: 1px solid #e0e0e0; margin: 12px 0;">
              <p style="margin: 0 0 4px; color: #555;">${ticketLabel}${tierName ? ` — ${tierName}` : ''}</p>
              <p style="margin: 0; font-weight: bold; color: #333;">Total paid: ${total}</p>
            </div>
            <div style="text-align: center; margin: 30px 0;">
              <a href="${walletLink}" style="display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; text-decoration: none; padding: 15px 40px; border-radius: 8px; font-weight: bold; font-size: 16px;">View My Tickets</a>
            </div>
            <p style="color: #666; font-size: 14px;">Your QR code${quantity > 1 ? 's are' : ' is'} available in your ticket wallet — show it at the door.</p>
            <hr style="border: none; border-top: 1px solid #e0e0e0; margin: 30px 0;">
            <p style="color: #999; font-size: 12px; text-align: center;">
              This email was sent by ${APP_NAME}. If you have any questions, please contact our support team.
            </p>
...
${ticketLabel}${tierName ? ` — ${tierName}` : ''}
Total paid: ${total}

View your tickets: ${walletLink}

Your QR code${quantity > 1 ? 's are' : ' is'} available in your ticket wallet — show it at the door.
...
This email was sent by ${APP_NAME}.
    ...
    if (error) {
      console.error('[EMAIL] Resend error (ticket purchase):', JSON.stringify(error, null, 2));
      return false;
    }

    console.log('[EMAIL] Ticket purchase confirmation sent. ID:', data?.id, '| To:', to);
    return true;
  } catch (error) {
    console.error('Exception sending ticket purchase confirmation email:', error);
    return false;
  }
}
